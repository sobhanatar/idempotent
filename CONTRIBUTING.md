...
* Please follow
  ...
  .
* Ensure that the current tests pass, and if you've added something new, add the tests where relevant. Your patch won't
  be accepted if it requires tests.
* Remember that we try to follow [SemVer](http://semver.org). If you are changing the behaviour, or the public api, you
  may need to update the docs. Randomly breaking public APIs is not an option.
* Send one pull request per feature. If you want to do more than one thing, send multiple pull requests.
* Send a coherent commit history, making sure each individual commit in your pull request is meaningful. If you had to
